...
def transToLsts(mat, mask=False, norm=False):  # 函数的作用就是提供tf.sparse.SparseTensor(idx, data, shape)这个方法需要的三个变量
...
class DataHandler:
	...
	def LoadData(self):
		...
		# 	tp = pd.read_csv(path, sep=' ',  names=['uid', 'iid'])
		# 	# print(tp[:20])
		# 	tp['uid'] -= 1
		# 	tp['iid'] -= 1
		...
		# 	mat = sp.csr_matrix(
		# 		(np.ones_like(rows), (rows, cols)), dtype='float64', shape=(args.user, args.item))
		# 	trnMats.append(mat)
		# self.trnMats = trnMats
		
		# path = self.tstfile + 'buy'
		# tp = pd.read_csv(path, sep=' ',  names=['uid', 'iid'])
		# tp['uid'] -= 1
		# tp['iid'] -= 1
		# tp = tp.sort_values("uid")
		# self.tstUsrs = np.unique(tp['uid'])
		
		# # self.tstDict = tp.groupby('uid')['iid'].apply(list).to_dict()
		# self.tstInt = tp.groupby('uid')['iid'].apply(list).to_dict()       # 为了对应原文的tst_int    得到唯一的uid对应iid
...
if __name__ == '__main__':
    data_handler = DataHandler()  # 使用小写的变量名，避免与类名冲突
