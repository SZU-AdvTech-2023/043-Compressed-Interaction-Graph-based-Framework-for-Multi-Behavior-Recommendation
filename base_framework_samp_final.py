...
class Recommender:
    ...
    def _init_weights(self):
        all_weights = dict()
        initializer = tf.random_normal_initializer(stddev=0.01)    # 使用标准差为0.01的正态分布

        self.weight_size_list = [args.latdim // 2] + [args.latdim // 2] * args.gnn_layer   # 结果是一个维数组，个数为4
...
if __name__ == '__main__':
    ...
    # datetime.datetime.now()的输出：YYYY-MM-DD HH:MM:SS.mmmmmm(后面是一长串m是微秒)

    # current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")   这行代码可以控制到秒，不要微秒
...
# for arg in vars(args):
#  这里的 vars(args) 会返回 args 对象的属性字典，这是一个循环，用于遍历 args 对象的所有属性。
#  print(arg + '=' + str(getattr(args, arg)))
# 对于 args 的每个属性，这行代码都会打印出属性名和其对应的值。getattr(args, arg) 用于获取 args 对象中名为 arg 的属性的值。    
